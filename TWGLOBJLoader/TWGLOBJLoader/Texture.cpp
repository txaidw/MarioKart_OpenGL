...
bool LoadTGA(Texture * texture, char * filename)				// Load a TGA file
{
	...
	{
		printf("Could not open texture file", filename);	// Display an error message
		return false;														// Exit function
	}

	if(fread(&tgaheader, sizeof(TGAHeader), 1, fTGA) == 0)					// Attempt to read 12 byte header from file
	{
		printf("Could not read file header", filename);		// If it fails, display an error message
	...
	{
		printf("TGA file be type 2 or type 10 ", filename);	// Display an error
		fclose(fTGA);
		return false;																// Exit function
	}
...
{																			// TGA Loading code nehe.gamedev.net)
	...
	{
		printf("Invalid texture information", filename);	// Display Error
	...
	{
		printf("Could not allocate memory for image", filename);	// Display Error
...
bool LoadCompressedTGA(Texture * texture, char * filename, FILE * fTGA)		// Load COMPRESSED TGAs
{
	...
	do
	{
		...
		{
			printf("Could not read RLE header", filename);	// Display Error
			if(fTGA != NULL)												// If file is open
			{
				fclose(fTGA);												// Close file
			}
		...
		{																	// that follow the header
			...
			for(short counter = 0; counter < chunkheader; counter++)		// Read RAW color values
			{
				...
				{
					printf("Could not read image data", filename);		// IF we cant, display an error
				...
				{
					printf("Too many pixels read", filename, NULL);			// if there is too many... Display an error!
		...
		else																			// chunkheader > 128 RLE data, next color reapeated chunkheader - 127 times
		{
			...
			{
				printf("Could not read from file", filename);			// If attempt fails.. Display error (again)
