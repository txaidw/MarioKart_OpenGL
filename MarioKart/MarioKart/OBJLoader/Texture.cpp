...
bool LoadTGA(Texture * texture, char * filename)				// Load a TGA file
{
	...
	{
		printf("Could not open texture file");	// Display an error message
		return false;														// Exit function
	}

	if(fread(&tgaheader, sizeof(TGAHeader), 1, fTGA) == 0)					// Attempt to read 12 byte header from file
	{
		printf("Could not read file header");		// If it fails, display an error message
	...
	{
		printf("TGA file be type 2 or type 10 ");	// Display an error
		fclose(fTGA);
		return false;																// Exit function
	}
